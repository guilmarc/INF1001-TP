...
function triertableau(tableau, clef) {
    switch (clef) {
        ...
        case "numofsub":
            tableau.sort(function compareActivity(a, b) {
                if (a.numofsub < b.numofsub) { return -1; }
                if (a.numofsub > b.numofsub) { return 1; }
                return 0;
            });
            break;
...
//fonction qui incrit les données du tableau selon la colonne cliquée par l'usager
function remplirTableau(column) {
    var i, rows, data = [
        ...
        tableauTri; // contiendra le tableau trié
    ...
    for (i = 1; i < 7; i++) {
        rows = document.getElementsByTagName("tr").item(i);
